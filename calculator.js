...
function calculator(button) {
    if (button.type == "operator") {
        ...
        BASES.forEach(base => {
            ...
            fromula_str = fromula_str.replace(toReplace, replacement);
        })
        ...
        NUMBERS.forEach(factorial => {
            fromula_str = fromula_str.replace(factorial.toReplace, factorial.replacement);
        })
        console.log(fromula_str);
        // CALCULATE
        let result;
        try {
            result = eval(fromula_str);
        }
        catch (error) {
...
function factorialNumberGetter(formula, FACTORIAL_SEARCH_RESULT) {
    let numbers = []; //Save all the numbers in same array
    let factorial_sequence = 0;
    FACTORIAL_SEARCH_RESULT.forEach(factorial_index => {
        let number = []; //current factorial number here
        ...
        //IF THERE IS A FACTORIAL SEQUENCE , WE NEED TO GET
        // THE INDEX OF VERY FIRST FACTORIAL FUNCTION
...
function powerBaseGetter(formula, POWER_SEARCH_RESULT) {
    let powers_bases = []; //save all power bases in the same array
